package com.aghnavi.agh_navi.dmsl.sensors;
...
public class SensorsMain implements SensorEventListener {
    ...
    private List<IOrientationListener> ioOrientationListeners = new ArrayList<>(2);

    public void addListener(IOrientationListener list) {
        if (!ioOrientationListeners.contains(list))
            ioOrientationListeners.add(list);
    }

    //NEVER USED
    public void removeListener(IOrientationListener list) {
        ioOrientationListeners.remove(list);
    }

    private void triggerOrientationListeners(float[] values) {
        for (IOrientationListener l : ioOrientationListeners) {
    ...
    interface IAccelerometerListener {
        void onNewAccelerometer(float[] values);
    }
     ...
     * Constructor
     *
     * @param context the activity it belongs too.
     */
    ...
    private void registerSensors() {
        ...
        if (!Objects.equals(orientation.getName(), "CWGD 9-axis Orientation Sensor")) {
    ...
    public float[] getMagnetism() {
        return new float[]{ mUserData.mag[0], mUserData.mag[1], mUserData.mag[2] };
    }
